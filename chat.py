...
class MiniCPMV2_6:
    ...
    def chat(self, input):
        ...
        if "image" in input and len(input["image"]) > 10:  # legacy API
            try:
                image = Image.open(
                    io.BytesIO(base64.b64decode(input["image"]))
                ).convert("RGB")
            except Exception as e:
                return "Image decode error"
            ...
            for c in contents:
                if isinstance(c, dict):
                    if c["type"] == "text":
                        ...
                        c = Image.open(
                            io.BytesIO(base64.b64decode(c["pairs"]))
                        ).convert("RGB")
                    else:
...
class MiniCPMVChat:
    """
    General MiniCPMV Chat class.
    """

    def __init__(self, model_path, multi_gpus=False, model=None) -> None:
        if model is not None:
    ...
    def chat(self, input):
        """
        Wrapper for chat function from model.
        """
